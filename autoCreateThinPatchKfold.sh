...
do
    ...
    if [ $MODELWEIGHT_DIRECTORY = "No" ];then
        modelweight_path="No"
    else
    ...
    do
        ...
        if [ $IS_LABEL = "No" ];then
        echo "IS_LABEL:No"
        python3 createThinPatch.py ${image_path} ${modelweight_path} ${save_path} --label_patch_size ${LABEL_PATCH_SIZE} --plane_size ${PLANE_SIZE} --overlap ${OVERLAP} --num_down ${NUM_DOWN} --num_channel ${NUM_CHANNEL} --num_rep ${NUM_REP}
        else
        echo "IS_LABEL:Yes"
        python3 createThinPatch.py ${image_path} ${modelweight_path} ${save_path} --label_patch_size ${LABEL_PATCH_SIZE} --plane_size ${PLANE_SIZE} --overlap ${OVERLAP} --num_down ${NUM_DOWN} --is_label --num_channel ${NUM_CHANNEL} --num_rep ${NUM_REP}
        fi
