...
class ThinPatchCreater():
    def __init__(self, image, model_path, label_patch_size, plane_size, overlap, num_down, num_channel=-1, num_rep=1, is_label=False, mask=None):
        self.image = image
    ...
    def execute(self):
        ...
        if mask is not None:
            mask = padding(mask, lower_pad_size[0].tolist(), upper_pad_size[0].tolist())
        print("Image shape : {}".format(image.GetSize()))
